...
<template>
  <div class="board h-screen overflow-auto px-3 py-3 scrollbar scrollbar-thumb-gray-600 scrollbar-thin scrollbar-track-gray-900" @click.self="formVisibility(false)">
    <div class="flex justify-start items-start gap-5">
      <div class="min-w-[280px] bg-gray-300 p-2 rounded-lg cursor-pointer" v-for="(column, colIndex) in trelloBoard.columns" :key="colIndex">
        <div class="flex justify-between mb-2 px-2">
          <h3 class="font-bold text-cyan-900">{{ column.name }}</h3>
          <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-gray-500" viewBox="0 0 20 20" fill="currentColor">
            <path d="M6 10a2 2 0 11-4 0 2 2 0 014 0zM12 10a2 2 0 11-4 0 2 2 0 014 0zM16 12a2 2 0 100-4 2 2 0 000 4z" />
          </svg>
        </div>
        <div class="bg-white px-2 pb-[5px] pt-2 rounded flex justify-between group drop-shadow-md" v-for="(task, taskIndex) in column.tasks" :key="task.id">
      ...
      <div class="min-w-[280px] bg-gray-200 p-2 rounded cursor-pointer text-gray-600">
        ...
        <template v-else>
          <input type="text" ref="formInp" v-model="newColumnName" placeholder="Enter list title..." @keyup.enter="addColumn" class="px-3 py-2 text-sm w-full focus:outline-blue-600">
          <div class="flex w-full mt-2 place-items-center space-x-2 transition-all ease-out duration-50" :class="!showlistControl ? 'h-0 overflow-hidden' : 'h-8'"  >
            <button @click="addColumn" class="bg-blue-600 py-2 px-[10px] rounded-sm text-sm text-white hover:bg-blue-700 transition-colors ease-out duration-300">Add list</button>
