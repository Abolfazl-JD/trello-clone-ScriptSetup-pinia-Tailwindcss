...
const changeNewList = (val: boolean) => {
...
<template>
  <div class="board h-screen overflow-auto px-3 py-3 scrollbar scrollbar-thumb-gray-600 scrollbar-thin scrollbar-track-gray-900" @click.self="changeNewList(false)">
    <div class="flex justify-start items-start gap-5">
      ...
      <div class="min-w-[280px] bg-gray-200 p-2 rounded cursor-pointer text-gray-600">
        <p class="w-full h-full" @click="changeNewList(true)" v-if="!addNewList">+ Add another list</p>
        <template v-else>
          ...
          <div class="flex w-full mt-2 place-items-center space-x-2 transition-all ease-out duration-50" :class="!showlistControl ? 'h-0 overflow-hidden' : 'h-8'"  >
            ...
            <svg xmlns="http://www.w3.org/2000/svg" @click="changeNewList(false)" class="h-6 w-6 text-gray-700 transition-colors ease-out duration-200 hover:text-gray-900" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
